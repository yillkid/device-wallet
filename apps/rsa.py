...
def encrypt_with_pub_key(pub_key, data):
    ...
    # Decrypt
    # private_key = ""
    # with open("accounts/john/private.pem", 'r') as outfile:
    ...
    # encrypted1 = encrypted.encode('ascii')

    # Hex to bin
    # encrypted1 = hex2bin(encrypted1)
    # decrypted1 = decryptor.decrypt(encrypted1)
    # print('Decrypted:', decrypted1.decode())
...
def sign_the_credential(user, credentials):
    private_key = ""
    with open("accounts/" + user + "/private.pem", 'r') as outfile:
        private_key = outfile.read()
    ...
    digest = SHA256.new()
    digest.update(str(credentials).encode())
